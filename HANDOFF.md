...
**Status:** T1.3 complete — VADER baseline (`feat: vader-baseline`, plus a same-day follow-up fix to `config.py`). `baselines/vader.py` runs `vaderSentiment`'s compound score (bundled lexicon, no `nltk.download` at runtime) over raw, unmodified sentences from the pinned test split, thresholded at ±0.05 (`config/default.yaml` → `vader.positive_threshold` / `negative_threshold`, not hardcoded). `evaluation/metrics.py` has `accuracy`, `macro_f1`, `weighted_f1`, `per_class_recall`, and `confusion_matrix`, built on a shared `per_class_stats` core (pure, tested without going through the CLI) — the exact shape T1.5's Done-when needs ("one row per method × {accuracy, macro-F1, weighted-F1, per-class recall}"), so T1.5 extends this module with the floor rows rather than rewriting it. `cli.py`'s `eval` command holds no logic beyond `--model` dispatch. `newsmood eval --model vader` prints all of accuracy, macro-F1, weighted-F1, per-class recall, and the 3×3 confusion matrix. Result: **54.05% accuracy, 45.83% macro-F1, 55.29% weighted-F1 — below the 62.1% majority-class floor**, with per-class recall as low as 22.22% on negative.

`docs/baselines.md` has all of it, plus two rounds of real misclassified examples: three from the original scan (VADER's lexicon scores "profit"/"loss" as fixed-polarity words and can't read the direction word next to them — "profit ... down from", "profit decreased", "loss narrowed"), and three more targeting the largest single confusion-matrix cell (true: neutral, predicted: positive — 126/518 rows), each driven by one finance noun as the sentence's only lexicon hit: "shares" (compound 0.1531), "profit" (0.4404), "growth" (0.3818).

While wiring `predict_test_split(settings)`, found `config.py`'s settings precedence didn't actually match the stated rule (explicit kwargs > env > yaml > code default) — the yaml file outranked explicit constructor kwargs entirely, so `Settings(dataset=DatasetSettings(splits_dir="/tmp/x", ...))` in a test silently came back as the real `data/splits`. First attempt at a fix only moved `init_settings` above yaml and left `env_settings` ahead of it, still wrong for the same reason one tier up — caught on review and corrected the same day. Now the actual order (`init_settings, env_settings, YamlConfigSource, dotenv_settings, file_secret_settings`) is asserted directly in `tests/test_config.py`, pairwise per tier, so a future reorder breaks a test instead of getting caught incidentally by whichever task happens to resolve a settings-derived path first. Full suite passes, including `test_phrasebank.py` (reads `splits_dir` and `seed` from settings), and `newsmood eval --model vader` / `python -m newsmood.data.phrasebank --summary` give identical output to before either reorder.
...
- `config.py`'s settings source order didn't match its own documented precedence (explicit kwargs > env > yaml > code default) — see §9 2026-09-18. Now covered by `tests/test_config.py`.
...
**Next task:** T1.4 — TF-IDF + Logistic Regression baseline, Monday Sep 21, 2026. Full calendar in `IMPLEMENTATION_GUIDE.md` §0.5.
