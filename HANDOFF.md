...
*Last updated: Saturday, Sep 19, 2026*

**Status:** T1.3 complete — VADER baseline (`feat: vader-baseline`, plus a same-task follow-up fix to `config.py`). `baselines/vader.py` runs `vaderSentiment`'s compound score (bundled lexicon, no `nltk.download` at runtime) over raw, unmodified sentences from the pinned test split, thresholded at ±0.05 (`config/default.yaml` → `vader.positive_threshold` / `negative_threshold`, not hardcoded). `evaluation/metrics.py` has accuracy, macro-F1, and confusion-matrix functions (pure, tested without going through the CLI) — minimal set for T1.3; T1.5 extends it with weighted-F1, per-class recall, and the floor rows. `newsmood eval --model vader` prints all three. Result: **54.05% accuracy, 45.83% macro-F1 — below the 62.1% majority-class floor.** `docs/baselines.md` has the confusion matrix and three real misclassified sentences, all the same failure mode: VADER's lexicon scores "profit"/"loss" as fixed-polarity words and can't read the direction word next to them ("profit ... down from", "profit decreased", "loss narrowed").

`config.py`'s settings precedence is now **explicit kwargs > env > yaml > code default**, directly tested in `tests/test_config.py` (pairwise per tier, plus a throwaway `BaseSettings` probe for the field-default tier, since no real settings field has one). The first fix (2026-09-18) only moved `init_settings` above yaml and left `env_settings` ahead of it, which was still wrong — corrected 2026-09-19 after catching it on review. Full suite (34 tests, all offline) passes, including `test_phrasebank.py`, and `newsmood eval --model vader` / `python -m newsmood.data.phrasebank --summary` give identical output to before either reorder.
...
- `config.py`'s settings source order needed two passes to actually match the stated precedence (explicit kwargs > env > yaml > code default) — see §9 2026-09-18 and 2026-09-19 rows. Now covered by `tests/test_config.py` so a future reorder breaks a test instead of getting caught incidentally.
