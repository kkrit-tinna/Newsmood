...
| 2026-09-18 | T1.3 | `config.py`'s `settings_customise_sources` reordered so `init_settings` outranks `YamlConfigSource` | pydantic-settings ranks sources by position (earlier wins); the T1.1 order (`env, yaml, init, ...`) meant any field `config/default.yaml` defines silently overrode explicit constructor kwargs, e.g. `Settings(dataset=DatasetSettings(splits_dir="/tmp/x", ...))` returned `data/splits` regardless. T1.2's tests never surfaced this because they always passed explicit `Path` args around the settings object instead of reading a settings-derived path back out. `baselines/vader.py`'s `predict_test_split(settings)` is the first function to resolve a path from `settings` the way the CLI does, and it silently loaded the real 518-row test split instead of a tmp fixture. First fix moved `init_settings` above yaml but left `env_settings` first — corrected 2026-09-19 (below). |
| 2026-09-19 | T1.3 | `config.py`'s source order corrected again: `init_settings` now outranks `env_settings` too (`init, env, yaml, dotenv, file_secret`) | The 2026-09-18 fix still put `env_settings` ahead of `init_settings`, so a stray `NEWSMOOD_*` env var in the test process could silently override an explicit test kwarg — the same class of bug, one tier up. Precedence is now explicit kwargs > env > yaml > code default, tested directly in `tests/test_config.py` (pairwise, plus a `field-default` tier probed via a throwaway `BaseSettings` subclass since no real settings field currently has a Python-level default). Full suite re-run after the change, including `tests/test_phrasebank.py` (reads `splits_dir` and `seed` from settings) — all 34 tests pass, `newsmood eval --model vader` and `python -m newsmood.data.phrasebank --summary` produce identical output to before the reorder. |
