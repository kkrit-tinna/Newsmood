...
│   │   ├── vader.py
│   │   ├── logreg.py
│   │   └── reference.py        # majority-class + stratified-random floors (T1.5)
│   ├── evaluation/
│   │   ├── metrics.py          # ported from the archived repo
│   │   ├── suite.py            # runs every baseline, merges into one method-keyed table (T1.5)
│   │   ├── baselines_doc.py    # writes the summary table into docs/baselines.md (T1.5)
│   │   └── gates.py
│   ├── reporting/
...
| 2026-09-22 | T1.5 | `newsmood eval --all` replaces only the region between `<!-- eval:summary:start/end -->` markers in `docs/baselines.md`, not the whole file | T1.3's `docs/baselines.md` already carries hand-written failure-mode analysis (three worked misclassification examples, a "why it fails" section) beyond what T1.3's own Done-when required. The guide's phrasing ("`newsmood eval --all` writes `docs/baselines.md`") reads as whole-file generation, which would silently delete that prose on every rerun. Scoped the writer to a marker-delimited summary table instead — idempotent, and safe for T1.6's writeup and any later rerun (e.g. after T2.3 adds rows) to coexist with hand-written sections. |
