...
def _population_size(table: dict[str, dict]) -> int:
    """Every method here is measured on the same pinned test split, so any
    one row's confusion matrix gives the population size."""
    first = next(iter(table.values()))
    return sum(sum(row) for row in first["confusion"])


def render_summary_table(table: dict[str, dict], human_ceiling_note: str, seed: int) -> str:
    ...
    `seed` is `dataset.seed` — stratified random draws once from it rather
    than averaging over repeats, so the doc has to say which seed a reader
    needs to reproduce the number."""
    n = _population_size(table)
    lines = [
        _START_MARKER,
        f"All rows below are measured on the pinned test split (n={n}).",
        "",
    ...
    lines.append("")
    lines.append(
        f"\"Stratified random\" is a single seeded draw from the training label "
        f"distribution (`dataset.seed={seed}`), not an average over repeats — "
        f"rerun with the same seed to reproduce it exactly."
    )
    lines.append(_END_MARKER)
    return "\n".join(lines)


def update_baselines_doc(path: Path, table: dict[str, dict], human_ceiling_note: str, seed: int) -> None:
    ...
    new_table = render_summary_table(table, human_ceiling_note, seed)
